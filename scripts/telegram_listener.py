...
"""
    ...
    prompt = (
        ...
        "**🚫 Meta 금융 정책 회피 (절대 준수):**\n"
        "Threads 는 트레이딩 시그널처럼 보이는 글을 자동 삭제함. style-guide.md 의\n"
        "'Meta 금융 콘텐츠 정책 회피' 섹션 규칙을 반드시 준수.\n"
        "- 자산명 + 시간프레임 + 방향 + 매매액션 4요소가 한 글에 모두 들어가면 안 됨.\n"
        "  최소 1~2개는 일반화/제거. 시간프레임은 '단기 차트'/'상위 프레임' 으로 표현 권장.\n"
        "- '롱 받아', '숏 진입', '여기서 들어가', '손절 필수', '익절 타겟' 같은\n"
        "  매매 행동 지시 표현 절대 금지. 관찰자 시점('관찰 중', '지켜볼 자리') 사용.\n"
        "- '참고만 해', '투자 권유 아님' 같은 면책 문구는 오히려 분류기에 시그널이 되니 쓰지 말 것.\n"
        "- T 캔들 시그널 이름은 체인 전체에서 1~2회만 언급, 반복 금지. 가능하면 '단기 신호'로 일반화.\n"
        "- 구체적 손절/익절 가격, '여기서 들어가면' 류 행동 가이드 출력 금지.\n\n"
        "**차트 시그널 해석 규칙 (매우 중요):**\n"
